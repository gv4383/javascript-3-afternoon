...
//Code Here
let evenNumbers = mixedNumbers.filter((val, i, arr) => val % 2 === 0);
...
//Code Here
let postTaxPrices = prices.map((val, i, arr) => val += val * 0.07);
...
let ordersTotal = orders.map((val, i, arr) => val["price"] * val["tax"] + val["price"]);
...
let bobsTotal = purchases.filter((val, i, arr) => val["owner"] === "Bob").reduce((total, val, i, arr) => total + val["price"], 0);
